import Scraper from "~/serveless/libs/Scraper";
...
export default class MangaModel extends Scraper {
    private static instance: MangaModel;

    private constructor(baseURL: string, axiosConfig?: AxiosRequestConfig, timeout?: number) {
        super(baseURL, axiosConfig, timeout);
    }

    public static getInstance(baseURL: string, axiosConfig?: AxiosRequestConfig, timeout?: number) {
        if (!this.instance) {
    ...
    public async getComic(mangaSlug: string) {
        ...
        try {
            const {data} = await this.client.get(
                `${this.baseURL}/${baseSlug}/${mangaSlug}`,
            );
