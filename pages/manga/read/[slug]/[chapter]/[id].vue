...
const {
  ...
  data: chapters,
  refresh,
} = await useLazyAsyncData('read-manga', () => $fetch(`/api/chapters?slug=${realSlug}&chapter=${chapterNumber.value}&id=${chapterID}`));
      ...
      <section class="flex h-fit relative">
        <div class="relative flex h-fit flex-1 text-white">
          <div class="h-fit min-h-screen w-full bg-black">
            ...
            <!--            <LazyMangaChapterHeader :refresh="refresh()" :chapter="chapter" :title="chapters.data[0].title"/>-->
