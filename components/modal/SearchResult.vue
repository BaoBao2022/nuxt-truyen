...
<template>
  <div class="min-h-fit w-full">
    <ul class="h-fit w-full space-y-5">
      <div class="lazyload-wrapper" v-for="manga in searchData">
        <li class="h-fit overflow-x-hidden bg-secondary py-4">
          <LazyNuxtLink class="flex h-full space-x-2" :to="useMangaDetailPagePath(manga.slug)" @click="close">
            <figure class="aspect-w-3 relative mt-4 ml-4 h-[120px] w-[90px] min-w-[85px] overflow-hidden rounded-xl">
              <span
                ...
                <img alt="img-preview" :src="manga.thumbnail" decoding="async" data-nimg="fill"
                     class="absolute inset-0 object-cover object-center" sizes="100vw" :srcset="manga.thumbnail"
            ...
            <div class="flex h-full flex-1 flex-col space-y-4 text-white transition-all hover:text-primary">
              <h2 class="mx-4 mt-4 w-full overflow-hidden font-secondary text-2xl line-clamp-1 md:text-4xl">
                {{ manga.name }}
              </h2>
