...
import SwiperItemLoading from "~/components/section/SwiperItemLoading.vue";
import SwiperBannerLoading from "~/components/section/SwiperBannerLoading.vue";
import SwiperCard from "~/components/section/SwiperCard.vue";
import SwiperCardButton from '~/components/section/SwiperCardButton.vue'
...
const mangaActive = ref<Manga | undefined>();
const activeIndexChange = (slide) => {
  mangaActive.value = mangas.value[slide.realIndex];
}
...
<template>
  <div class="w-full">
    <div class="group relative w-full md:h-[350px] xl:h-[400px] 2xl:h-[450px] overflow-hidden">
      <SwiperBannerLoading v-if="pending"/>
      <template v-else>
        <div class="w-full h-0" v-if="mangaActive && mangaActive.thumbnail">
          ...
          <SwiperCard :spotlight="mangaActive"/>
        </div>
        ...
        <div
          ...
          <h1 class="text-2xl font-bold uppercase md:text-4xl sm:line-clamp-3 md:line-clamp-4 line-clamp-1">
            {{ mangaActive?.name }}
          </h1>
          <div class="flex flex-wrap items-center mt-4 text-lg gap-x-8">
            ...
            <div class="flex line-clamp-1 items-center space-x-2 text-xl">
              <span v-for="genre in mangaActive?.genres">{{ genre }}</span>
            </div>
          </div>
          <div
              class="m-h-[160px] prose !max-w-full hidden md:block text-gray-200 md:line-clamp-4 lg:line-clamp-5 text-xl">
            {{ mangaActive?.review }}
            <br>
            <br>
            Author: {{ mangaActive?.author ? mangaActive?.author : 'N/a' }}
          </div>
          <SwiperCardButton :spotlight="mangaActive"/>
        </div>
    ...
    <div class="w-full px-4 md:px-12 mt-4">
      <div
        ...
        <SwiperItemLoading v-if="pending"/>
        <template v-else>
          <ClientOnly>
            <Swiper
                :navigation="true"
          ...
          <div slot="container-end" class="swiper-navigation absolute right-0 bottom-full mb-4 flex space-x-4">
            <button type="button"
                    class="transition duration-300 p-2 rounded-xl-full swiper-button-prev flex items-center justify-center hover:bg-white hover:text-black border-solid border-2 border-white gap-x-2">
            ...
            <button type="button"
                    class="transition duration-300 p-2 rounded-xl-full swiper-button-next flex items-center justify-center hover:bg-white hover:text-black border-solid border-2 border-white gap-x-2">
              <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round"
                   stroke-linejoin="round" class="w-6 h-6" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
