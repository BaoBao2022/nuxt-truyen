...
export default class Lazy {
    ...
    public mount(
        ...
        if (!el) return;
        const { src, loading, error, lifecycle, delay } = this._valueFormatter(
    ...
    public update(
        ...
        const { src, error, lifecycle, delay } = this._valueFormatter(
            typeof binding === "string" ? binding : binding.value
        );
        this._initIntersectionObserver(el, src, error, lifecycle, delay);
    }
    ...
    private _setImageSrc(
        ...
        if (el.tagName.toLowerCase() === "img") {
            if (src) {
                ...
                if (preSrc !== src) el.setAttribute("src", src);
            }
        ...
        this._images.set(
            ...
            new IntersectionObserver((entries) => {
                Array.prototype.forEach.call(entries, (entry) => {
                    if (delay && delay > 0)
                        this._delayedIntersectionCallback(
                            el,
                            entry,
                            delay,
                            src,
                    ...
                    else this._intersectionCallback(el, entry, src, error, lifecycle);
                });
            }, observerOptions)
        );
     ...
     * @memberof Lazy
     */
    public _valueFormatter(
        value: ValueFormatterObject | string
        ...
        if (isObject(value)) {
            ...
            lifecycle =
                (value as ValueFormatterObject).lifecycle || this.options.lifecycle;
